...
func (service RaoService) DocFilter(doc document.IDocument) bool {
	...
	log.Debug("Document path : " + doc.GetPath(), REFERER)
	log.Debug("Document title: " + doc.GetTitle(), REFERER)
	matches := RAO_PATTERN_FILTER.FindStringSubmatch(doc.GetPath())
	if nil == matches {
		return false
	}
	log.Debug("doc complies with filter assertion, processing: " + doc.GetPath() + doc.GetTitle(), REFERER)
	return true
}
...
func (service RaoService) DocHandler(doc document.IDocument) {
	...
	content := string(b[:])
	//log.Debug("content : " + content, REFERER)
	if "" == content {
